...
uint32_t Servo_AngleToUs(uint8_t angle)
{
    if (angle > 180U) angle = 180U;                       // clamp: 범위 초과 입력 방어
    return SERVO_MIN_US
...
volatile bool double_press = false;
...
int main(void)
{
  ...
  while (1)
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  //1타
	  if(isPressed == true && double_press == false)
	  {
		  now_time = HAL_GetTick();
		  if(now_time - pressTime > 1000 && isPressed == true)	//1초 이상 누르고 있다.
		  {
			  for(us; us < SERVO_MAX_US; us += 10u)
			  {
				  if(isPressed == false) break;
	  ...
	  //2타
	  else if(isPressed == true && double_press == true)
	  {
		  now_time = HAL_GetTick();
		  if(now_time - pressTime > 1000 && isPressed == true)	//1초 이상 누르고 있다.
		  {
			  for(us; us > SERVO_MIN_US; us -= 10u)
			  {
				  if(isPressed == false) break;
				  __HAL_TIM_SET_COMPARE(&htim4,TIM_CHANNEL_3,us);
				  HAL_Delay(20);
	  ...
	  {
		  now_time = HAL_GetTick();
		  if(now_time - releaseTime > 1000) 	//버튼에서 손 뗀지 1초 이상 지났다면
		  {
			  double_press = false;
		  }
	  }
