...
void DisplayLED(int led_number)
{
	...
	static int limit_of_blink = 0;     //최대 점멸 횟수 - 초당 점멸 횟수 제어용
	...
	if (CHECK_BIT(ShiftRegisterValue, check_bit))
	{
		...
		if(HAL_GetTick() - Last_LED_Toggle_Tick >= preiod_of_blink)
		{
			HAL_GPIO_TogglePin(GPIOB,GPIO_PIN_NUM);
			Last_LED_Toggle_Tick = HAL_GetTick();
			now_number_of_blink++;

			if(limit_of_blink <= now_number_of_blink)
			{
				now_number_of_blink = 0;
			}
		}
	...
	else
	{
		...
		HAL_GPIO_WritePin(GPIOB,GPIO_PIN_NUM,GPIO_PIN_RESET);
	}
}
...
int main(void)
{
  ...
  bool BTN_ONOFF = BTN_OFF;		   //지금 버튼의 상태가 On/Off인지
  bool blink = true;	   //LED가 점멸을 할지 쭉 켜져있을지
